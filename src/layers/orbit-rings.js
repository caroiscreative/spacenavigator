...
export function createOrbitRings(scene, tles) {
  ...
  for (let t = 0; t < tles.length; t++) {
    ...
    if (a_km < 6378 || a_km > 500_000) continue;

    const e   = Math.min(satrec.ecco, 0.9999);
    const b_km = a_km * Math.sqrt(1 - e * e);

    const i  = satrec.inclo;
    const Om = satrec.nodeo;
    const w  = satrec.argpo;
    ...
    for (let j = 0; j <= SEGS; j++) {
      const E   = (j / SEGS) * Math.PI * 2;
      const xp  = a_km * (Math.cos(E) - e);
      const yp  = b_km * Math.sin(E);
